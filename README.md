...
- **rally を通さず書いたコードも後追いで管理下に**:実装からテストの穴を棚卸しし、確認した振る舞いは正式テスト、未確認分は「特性化テスト」と明示して固定(現状追認を無自覚にやらない)
- **未定義の仕様は推測で埋めない**:実装・試験中に「どう動くべきか決まっていない」状況に出会うと、ミニインタビューでユーザーに戻ってくる
- **仕様書は基本設計書の形で 1 枚の HTML に**:E2E と ADR から `docs/spec.html` を生成。14 章の定義書(機能一覧・機能仕様・エラー仕様・非機能要件・構成図・処理フロー…)で、各項目にそれを固定している E2E 名と**確定／暫定／未固定**の判定が付く。「人が承認した仕様」と「現状を追認しただけの仕様」が一目で分かる
- **決定が自動で残る**:Stop hook が ADR(`docs/adr.md`)への追記を促す

## ワークフロー

```mermaid
flowchart TD
    A["一言の要望"] --> B["質問ラウンド<br/>(最大4問・確定/委任/保留)"]
    B -->|続ける| B
    ...
    I --> G
    J --> G
    G -.->|Stop hook| K["ADR の自動促し"]
```

### 各フェーズでやること

1. **質問ラウンド** — 観点表(7観点:入力異常系/境界/状態と順序/不変条件/解釈の言い直し/スコープ外/対立軸)と、要望固有の軸の表(質問前に AI が書き出す)から「答えによって実装が最も変わる質問」を最大 4 問選んで提示。回答は**確定**(仕様に固定)/**委任**(AI が仮決めして明記)/**保留**(モック・複数案で次ラウンドへ)に三分類。確定は**満たす例/満たさない例の対**で返す(解釈のズレは満たさない例でしか表に出ない)。続けるかまとめるかは毎ラウンド末にユーザーが決める
2. **要件の整合チェック** — まとめ指示を受けたら、ラウンド間の回答の矛盾・確定/委任/保留の反映漏れ・「なし」判断の見直し・仕様書の各章が書けるか(章別判定)・各要件に「誰が・いつ使うか」が書けるか(書けなければ過剰の疑い)を実施。問題があれば質問ラウンドに戻り、通過して初めて先へ進む
...
5. **実装+標準試験バッテリー** — 確定した Gherkin を E2E 化するのに加え、機械的試験 10 カテゴリ(境界値・入力異常系・データ頑健性・冪等性・状態遷移・往復一致・不変条件・出力契約・環境依存・スケール)から該当分を**確認なしで自動展開**。機械的な欠陥を人間に発見させないため。展開後は同梱の静的検査(記録に書いた試験名の実在・assert 無し・理由なし skip・例外の握り潰し)を実行し、green の後に実装を 1 か所壊して試験が赤くなることを確かめる(煙試験)
...
9. **Stop hook** — ターン終了時、ADR 未反映の決定があれば `docs/adr.md` の更新を 1 回だけ促す(rally 未使用プロジェクトでは沈黙)

## スキル一覧

| スキル | 内容 |
|---|---|
| `spec-interview` | 観点表に沿った質問ラウンド(1 ラウンド最大 4 問)で壁打ちし、回答を確定/委任/保留に三分類。まとめ指示で要件の整合チェック → 技術選定(脆弱性チェック付きでユーザーと合意)→ プラン提示。承認が実装開始の合図。実装時は標準試験バッテリーを自動展開 |
| `promote-bug` | バグを修正する前に再現試験を E2E 層へ昇格させる(失敗するのを確認してから直す)。期待挙動が未定義ならミニインタビューへ |
| `spec-digest` | E2E(仕様の正)と ADR から基本設計書 `docs/spec.html` を生成・再生成。ブラウザで開ける単一ファイルで、章の目次と全文検索が付く。**モデルが書くのは行形式の原稿だけで、HTML は 1 バイトも同梱スクリプトが組み立てる**。機能一覧・確定状況・件数・工程番号・判定はすべて自動導出で、同じ数字を 2 か所に書く場所が無い。章立てや ID の連番から外れるとビルドが行番号つきで落ちる |
| `backfill-tests` | rally を通さず作成・変更されたコードに不足テストを後追い補充。期待値は「仕様記録から導出/ユーザー確認済み/特性化テスト明示マーク」の三択で、推測の無印固定(現状追認)を禁止。確認済み Gherkin は仕様記録に合流。バグ確定挙動は「失敗を期待する」再現テスト(xfail 相当)で保存し、修正は promote-bug が引き継ぐ。実データは観測・テスト実行から隔離 |
...
Stop hook が、**rally のスキルを実際に使ったプロジェクトでのみ**、確定・仮決めした決定の `docs/adr.md` への一言追記を促します。

- rally 未使用のプロジェクトには一切干渉しません(判定は `docs/interviews/` の存在 = rally のスキル(spec-interview / backfill-tests)だけが作るディレクトリ。transcript に依存しないため、リモートコントロールセッションでも動作)
...
| ADR | `docs/adr.md` | 決定と理由・捨てた案・対立した要件の一言記録 |
| コード | `src/` など | 実装本体 |
| 観点表 | プラグイン同梱+`.claude/rally-checklist.md` | 質問の元ネタ。失敗のたびに育てる |

補助資料: `docs/interviews/`(壁打ちの記録=プラン)、`docs/packages/`(依存の脆弱性調査報告)、`docs/spec.html` と原稿 `docs/.spec-src.md`(spec-digest で生成する基本設計書)。
...
7. 「**仕様書を作って**」で `docs/spec.html` を生成 — ブラウザで開くと、サイドバーの目次と検索が付いた 14 章の基本設計書が読める
...
- **Python 3**(PATH 上に `python`)— Stop hook と仕様書(`docs/spec.html`)の生成に必要
