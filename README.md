...
1. **質問ラウンド** — 観点表(7観点:入力異常系/境界/状態と順序/不変条件/解釈の言い直し/スコープ外/対立軸)と、要望固有の軸の表(質問前に AI が書き出す)から「答えによって実装が最も変わる質問」を最大 4 問選んで提示。回答は**確定**(仕様に固定)/**委任**(AI が仮決めして明記)/**保留**(モック・複数案で次ラウンドへ)に三分類。続けるかまとめるかは毎ラウンド末にユーザーが決める
